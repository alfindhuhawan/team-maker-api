...
// WDR220119150807G0123AB

//WDR    : 3 digit static id identifier
...
type SellerWithdrawID string

func NewSellerWithdrawID(now time.Time, random6Char string) (SellerWithdrawID, error) {
	...
	return SellerWithdrawID(fmt.Sprintf("WDR%sX%s", now.Format("060102150405"), random6Char)), nil
}

func (n SellerWithdrawID) Validate() error {
	...
	if n[0:3] != "WDR" {
		return fmt.Errorf("id must started with WDR %s", n[0:3])
	}
